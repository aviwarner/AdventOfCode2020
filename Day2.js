...
const countValidPasswords = passwordArray => {
    let validPasswordCount = 0;

    passwordArray.forEach(password => {
        if (checkValidPassword(password)) validPasswordCount++;
    });
...
const checkValidPassword = password => {
    const policy = password[0];
    const letter = password[1];
    const passwordToCheck = password[2];

    const policyArr = policy.split('-');
    const min = parseInt(policyArr[0], 10);
    const max = parseInt(policyArr[1], 10);

    let targetCount = 0;

    passwordToCheck.split('').forEach(char => {
        if (char === letter) targetCount++;
    })

    return targetCount >= min && targetCount <= max;
}

console.log('Valid password count: ', countValidPasswords(INPUT)) // 524

// Day 2 - Star 2

const countValidPasswordsStar2 = passwordArray => {
    let validPasswordCount = 0;

    passwordArray.forEach(password => {
        if (checkValidPasswordIndeces(password)) validPasswordCount++;
    });

    return validPasswordCount;
}

const checkValidPasswordIndeces = password => {
    const policy = password[0];
    const letter = password[1];
    const passwordToCheck = password[2];

    const policyArr = policy.split('-');
    const index1 = parseInt(policyArr[0], 10) - 1;
    const index2 = parseInt(policyArr[1], 10) - 1;

    const splitPassword = passwordToCheck.split('');

    if (splitPassword[index1] === letter) {
        if (splitPassword[index2] === letter) return false;
        return true;
    } else if (splitPassword[index2] === letter) {
        if (splitPassword[index1] === letter) return false;
        return true;
    }
...
console.log('Valid password count (part 2): ', countValidPasswordsStar2(INPUT)) // 485
